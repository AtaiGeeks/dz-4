...
// ЗАДАНИЕ 1 — Получение персонажей и отрисовка карточек
...
fetch("../data/characters.json")
  ...
  .catch(error => console.error("Ошибка при загрузке персонажей:", error));

// ЗАДАНИЕ 2 — XMLHttpRequest на any.json и вывод в консоль
const xhr = new XMLHttpRequest();
xhr.open("GET", "../data/any.json");
xhr.onload = function () {
  if (xhr.status === 200) {
    const result = JSON.parse(xhr.responseText);
    console.log("Данные из any.json:", result);
  } else {
    console.error("Ошибка при загрузке any.json");
  }
};
xhr.send();
